...
public class SupabaseStorageService {
  ...
  public String uploadCover(MultipartFile file) throws IOException {
    ...
    String uploadUrl = "https://" + SUPABASE_PROJECT_ID + ".supabase.co/storage/v1/object/" + STORAGE_BUCKET + "/"
        + fileName;
    ...
    if (response.getStatusCode().is2xxSuccessful()) {
      ...
      throw new RuntimeException("Erro no upload: " + response.getBody());
    }
  }

  public void deleteCover(String fileName) {
    String deleteUrl = "https://" + SUPABASE_PROJECT_ID + ".supabase.co/storage/v1/object/" + STORAGE_BUCKET + "/"
        + fileName;
  ...
  public String replaceCover(String oldCoverUrl, MultipartFile newFile) throws IOException {
    ...
    String updateUrl = "https://" + SUPABASE_PROJECT_ID + ".supabase.co/storage/v1/object/" + STORAGE_BUCKET + "/"
        + fileName;

    HttpHeaders headers = new HttpHeaders();
    headers.setBearerAuth(SUPABASE_API_KEY);
    headers.set("apikey", SUPABASE_API_KEY);
    headers.setContentType(MediaType.parseMediaType(newFile.getContentType()));
    ...
    if (response.getStatusCode().is2xxSuccessful()) {
      ...
      throw new RuntimeException("Erro ao atualizar capa: " + response.getBody());
    }
  }
  ...
  public String extractFileNameFromUrl(String url) {
    ...
    // Exemplo:
    // https://project.supabase.co/storage/v1/object/public/bucket/covers/file.jpg
